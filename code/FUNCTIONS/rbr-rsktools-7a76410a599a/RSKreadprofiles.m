...
if strcmpi(direction{1}, 'both') % strcmpi is case insensitive, consistent with validatestring
    if any(strcmpi({RSK.regionCast.type},'down')) && any(strcmpi({RSK.regionCast.type},'up'))  
        if RSK.profiles.downcast.tstart(1) < RSK.profiles.upcast.tstart(1)
            direction = {'down', 'up'};
        else
            direction = {'up','down'}; 
        end
...
if size(RSK.profiles.order, 2) == 1
    ...
    if hasGPS
        ...
        lat2fill(1:2:end) = num2cell(RSK.profiles.GPS.latitude(round(castidx(1:2:end)/2)));
        for j = 2:2:length(lat2fill)
            lat2fill(j) = lat2fill(j-1);    
        end
       
        lon2fill(1:2:end) = num2cell(RSK.profiles.GPS.latitude(round(castidx(1:2:end)/2)));
