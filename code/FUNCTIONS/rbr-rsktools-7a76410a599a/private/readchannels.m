...
if any(strcmpi({tables.name}, 'instrumentChannels')) 
    ...
    columns = doSelect(RSK, 'PRAGMA table_info(channels)');
    if any(strcmp({columns.name},'unitsPlainText'))
        RSK.channels = doSelect(RSK, ['SELECT c.shortName as shortName,'...
                            'c.longName as longName,'...
                            'c.units as units, '...
                            'c.unitsPlainText as unitsPlainText,'...
...
if (~isfield(RSK,'channels')||isempty(RSK.channels))
    % check if unitsPlainText exists in channels
    columns = doSelect(RSK, 'PRAGMA table_info(channels)');
    if any(strcmp({columns.name},'unitsPlainText'))
        RSK.channels = doSelect(RSK,'SELECT shortName, longName, units, unitsPlainText, channelID FROM channels ORDER by channels.channelID');
    else
