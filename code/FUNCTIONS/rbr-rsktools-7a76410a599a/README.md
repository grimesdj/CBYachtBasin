...
Current stable version: 3.6.0 (2024-01-04)
...
* Check out [Getting started](https://rbr-global.com/wp-content/uploads/2024/01/GettingStarted.pdf)
  and [Post-processing](https://rbr-global.com/wp-content/uploads/2024/01/PostProcessing.pdf) for a quick start.
...
* Version 3.6.0 (2024-01-04)
    - RSKtools support the mksqlite MEX file for macOS with Apple silicon
    - New function `RSKderiveAPT` to derive accelerations and the temperature for the Triax accelerometer
    - RSKtools support case-insensitive input arguments
    - RSKtools support reading unitsPlainText field
    - Bug fix `RSKimages` regarding the selection of unit time for `showGap` option.
    - Bug fix `RSKopen` regarding `readHiddenChannels`
    - Bug fix `RSKcentrebursttimestamp` due to `samplingCount` field change in RSK schema
    - Bug fix for `channelID` updates for all derivation functions
    - Bug fix for `RSKbinaverage` to stop adding one more bin out of the boundary
    - Improve `RSKreadprofiles` to be independent of RSK.region.type
    - Bug fix for `RSKcomputeprofiles`, `RSKdetectprofiles` by considering NaN values in `pressure` channel.
    - Accommodate short name changes for RBRquartz3 Q `period` channels.   
