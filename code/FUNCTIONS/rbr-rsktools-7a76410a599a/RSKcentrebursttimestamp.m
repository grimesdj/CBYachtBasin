...
if isfield(RSK.schedules,'samplingCount')
    dt = (RSK.schedules.samplingCount/2)*readsamplingperiod(RSK)/86400;
    RSK.data.tstamp = RSK.data.tstamp + dt;
elseif isfield(RSK.wave,'samplingCount')
    dt = (RSK.wave.samplingCount/2)*readsamplingperiod(RSK)/86400;
    RSK.data.tstamp = RSK.data.tstamp + dt;
else
    RSKwarning('samplingCount cannot be found');
end
