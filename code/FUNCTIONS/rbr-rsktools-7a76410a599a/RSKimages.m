...
for c = RSK.im.channel
    ...
    if ~showgap
        ...
        % Sometimes profiles are not continuous in time, here recreate the
        % 2D(time * depth) matrix with x of continuous time and NaN when no
        % profiles measured
        unit_time = min(abs(x(1:end-1) - x(2:end)));% select the smallest time space between two profiles as unitTime
        N = round((x(end)-x(1))/unit_time);
        x_itp = linspace(x(1), x(end), N); % rebuild x with continuous time
        
        ind_mt = bsxfun(@(x,y) abs(x-y), x(:), reshape(x_itp,1,[]));
        [~, ind_itp] = min(ind_mt,[],2); 
        ind_nan = setxor(ind_itp, 1:length(x_itp));% get indices when no profiles measured 
