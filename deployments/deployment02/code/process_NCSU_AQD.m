...
inputDir  = '/Users/derekgrimes/OneDriveUNCW/Data/CB_YachtBasin/MarshMayhem/AQD13862/';
inputFile = 'CB_S202';
...
atmTime = [datenum('05/22/2025 14:00:00'), datenum('05/22/2025 14:30:00')];
depTime = [datenum('05/22/2025 15:30:00'), datenum('06/06/2025 14:00:00')];
...
if corrFlag
    ...
    A.qcFlag   =  double( (dum2<=dum1) & min(A.a1,min(A.a2,A.a3))>30 );
end
%
...
for j = 1:length(fieldNames)
    if ~ismember(fieldNames{j},originalFields)
        A.(fieldNames{j}) = A.config.(fieldNames{j});
    else
        disp(['field name: "',fieldNames{j},'" already exists'])
        preexistingFields = cat(1,preexistingFields,j)
    end
end
fieldNames(preexistingFields) = [];
